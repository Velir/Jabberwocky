...
namespace Jabberwocky.Glass.Factory.Exceptions
{
	public class ImplementationMismatchException : Exception
	{
		private const string DefaultPrescriptiveMessageFormat = 
			"The provided Glass Model type '{0}' does not match the type required by the target implementation type '{0}'.\n" + 
		...
		public ImplementationMismatchException(Type targetImplType, Type glassModelType)
		{
			_message = string.Format(DefaultPrescriptiveMessageFormat, glassModelType.Name, targetImplType.Name);
		}
